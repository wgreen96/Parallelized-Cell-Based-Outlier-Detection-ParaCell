    ...
    public void process(Context context,
        ...
        for(Hypercube currPoints: windowPoints){
            ...
            if(!hypercubeState.containsKey(currHypID)){
                ...
                cellIndices.put(currHypID, setOfMeanCoords.size()-1);
            }else{
                ...
                if(currTime > lastModification.get(currHypID)){
                    ...
                    setOfMeanCoords.set(indexToReplace, currPoints.centerOfCellCoords);
                }
            }
        ...
        //Sort setOfMeanCoords by the dimension in dimWithHighRange. Then sort by the last index which stores the number of data points in a hypercube
        ...
        for(int arrIndex = 0; arrIndex < setOfMeanCoords.size(); arrIndex++){
            ArrayList<Double> tempArrayList = setOfMeanCoords.get(arrIndex);
            ...
            if(cellDataCount < minPts){
                ...
                while(!stop){
                    ...
                    for(int j = currentSize-1; j >= 0; j--){
                        ...
                        ArrayList<Double> currCell = setOfMeanCoords.get(loopIndex);
                    ...
                    if(valsIndex.size() == 0){
                        ...
                        if((valsIndex.get(valsIndex.size()-1) + searchIndex + 1) >= setOfMeanCoords.size()){
